    ...
    */
	...
	async function pickAudio() {
		const path = await open({
            ...
            recursive: false,
            filters: [
				{extensions: ["mp3", "wav", "ogg"], name: $lang.modal.project_settings.audio_picker.filters.all_supported_audio_files},
				{extensions: ["mp3"], name: $lang.modal.project_settings.audio_picker.filters.mp3_audio_file},
				{extensions: ["wav"], name: $lang.modal.project_settings.audio_picker.filters.wav_audio_file},
				{extensions: ["ogg"], name: $lang.modal.project_settings.audio_picker.filters.ogg_vorbis_audio_file},
			],
        });
        ...
        if (path === null) {
			...
			try {
				...
				Log.ui.error("Failed to copy audio file to save folder:", e instanceof Error ? e.message : String(e));
