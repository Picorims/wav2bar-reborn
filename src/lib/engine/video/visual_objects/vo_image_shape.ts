...
export class VO_ImageShape implements VisualObjectRenderer<SaveVO_ImageShape> {
    ...
    private parseCSSGradient(gradient: string): FillGradient {
        ...
        if (subString.includes("linear-gradient") || subString.includes("radial-gradient")) {
            ...
            console.warn("CSS gradient: Multiple gradients defined in one string are not supported. Only the first gradient will be used.");
        ...
        if (subString.includes("rgb")) {
            ...
            for (const match of matches) {
                ...
                if (rgbaMatch) {
                    ...
                    const alphaHex: string = a < 1 ? Math.round(a * 255).toString(16).padStart(2, "0") : "";
                    const hexColor = `#${rgbHex}${alphaHex}`;
                    console.warn(`CSS gradient: RGB/RGBA color "${rgbString}" converted to hex format "${hexColor}". Only hex colors are supported in gradients.`);
        ...
        if (gradient.startsWith("linear-gradient(")) {
            ...
            if (params[0].includes("to ")) {
                ...
                console.warn("CSS gradient: Unsupported linear gradient syntax. Only 'to direction' and 'angle in deg' are supported.");
            ...
            for (let i = 1; i < params.length; i++) {
                ...
                if (!color || !offset) {
                    console.warn(`CSS gradient: Unsupported color stop syntax. Color stops should be defined as 'color offset%' (got: ${params[i]}).`);
            ...
            if (params[0].includes("ellipse") || params[0].includes("circle")) {
                console.warn("CSS gradient: using 'ellipse' or 'circle' syntax in radial gradients is not supported. Only position-based syntax is supported.");
            ...
            if (params[0].includes("at ")) {
                ...
                if (position === "center") {
                    ...
                    console.warn(`CSS gradient: Unsupported radial gradient position "${position}". Only "center" and "x% y%" formats are supported.`);
                    return defaultGradient; // unsupported position
                }
            } else {
                console.warn("CSS gradient: Unsupported radial gradient syntax. Only position-based syntax is supported.");
            ...
            console.warn("CSS gradient: Unsupported gradient type. Only linear-gradient and radial-gradient are supported.");
            return defaultGradient;
        }
    }
}
